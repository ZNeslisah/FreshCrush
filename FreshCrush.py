...
class FreshCrushGame(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Fresh Crush Game")
    ...
    def getCandies(self):
        ...
        ann_command = [
            'python3',
            f'{self.current_dir}/FreshClassify.py',
            selected_files_path,
    ...
    def find_matches_in_grid(self, grid):
        """Finds matches in a given grid and returns a set of positions to be cleared."""
        matches = set()

        # Check horizontal matches
        for r in range(self.rows):
            for c in range(self.cols - 2):
                if grid[r][c] == grid[r][c + 1] == grid[r][c + 2] and grid[r][c] != " ":
                    matches.add((r, c))
                    matches.add((r, c + 1))
                    matches.add((r, c + 2))
        ...
        for c in range(self.cols):
            for r in range(self.rows - 2):
                if grid[r][c] == grid[r + 1][c] == grid[r + 2][c] and grid[r][c] != " ":
                    matches.add((r, c))
                    matches.add((r + 1, c))
                    matches.add((r + 2, c))
    ...
    def solve(self):
        """Finds the best move considering cascading effects."""
        best_move = None
        max_score = 0  
        ...
        for r in range(self.rows):
            for c in range(self.cols):
                # Check right swap
                if c < self.cols - 1:
                    temp_grid = [row[:] for row in self.grid]  # Copy the grid
                    temp_grid[r][c], temp_grid[r][c + 1] = temp_grid[r][c + 1], temp_grid[r][c]  # Swap
                    score = self.simulate_cascading(temp_grid)
                ...
                if r < self.rows - 1:
                    temp_grid = [row[:] for row in self.grid]  # Copy the grid
                    temp_grid[r][c], temp_grid[r + 1][c] = temp_grid[r + 1][c], temp_grid[r][c]  # Swap
                    score = self.simulate_cascading(temp_grid)
    ...
    def simulate_cascading(self, temp_grid):
        """Simulates the full cascading process on a temporary board and returns the total score."""
        total_score = 0
        while True:
            matches = self.find_matches_in_grid(temp_grid)  # Use the fixed function
            if not matches:
                break  # Stop when no more matches occur

            total_score += len(matches)  # Add to total score
            for r, c in matches:
                temp_grid[r][c] = " "  # Clear the matched cells
    ...
    def solve_move(self):
        """Automatically solves the best move with cascading effect taken into account."""
        def solve_step():
            move = self.solve()
            if move:
                pos1, pos2 = move
                ...
                if matches:
                    ...
                    self.clear_matches(matches)
                    self.update_buttons()
                    QTimer.singleShot(300, solve_step)  # Delay for UI update and continue solving
                else:
    ...
    def clear_matches(self, matches):
        ...
        self.cascade_candies()  # Trigger the cascading effect after clearing


    def cascade_candies(self):
        """Handles cascading by making candies fall down into empty spaces without introducing new ones."""
        for c in range(self.cols):
            empty_rows = []

            # Collect empty rows in the column
            for r in range(self.rows - 1, -1, -1):
                if self.grid[r][c] == " ":
                    empty_rows.append(r)
                elif empty_rows:
                    # Move candy down to the lowest empty row
                    empty_row = empty_rows.pop(0)
                    self.grid[empty_row][c] = self.grid[r][c]
                    self.grid[r][c] = " "
                    self.image_mapping[(empty_row, c)] = self.image_mapping[(r, c)]
                    self.image_mapping[(r, c)] = ""

                    # Update the empty row list
                    empty_rows.append(r)

        self.update_buttons()

        # Check for new matches after cascading
        matches = self.find_matches()
        if matches:
            self.score += self.calculate_score(matches)
            self.score_label.setText(f"Score: {self.score}")
            self.clear_matches(matches)  # Recursively clear and cascade
